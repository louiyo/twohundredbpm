...
def load_csv_data(data_path, sub_sample=False, parameter='PRI_jet_num'):
    ...
    automatically sorted according to their jet_num value
    Arguments : data_path (string name of path to .csv file)
                sub_sample (boolean to do sub sampling)
                parameter (string name of parameter to sort particles)
    """
    
    # Separating x and y, and getting the PRI_jet_num column number
    ...
    # Extracting the different jet num values in a set
    values = set(input_values[:, parameter_id].reshape(-1,))  # [0, 1, 2, 3]
    
    
    # Triple iteration that gets the ids along with parameters and value
    data = [[i for i in zip(zip(input_values, yb), ids)
             if i[0][0][parameter_id] == x] for x in values]
    
    
    # Extracting the data from the zip iteration
