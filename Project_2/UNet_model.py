...
def conv_batch(img, n_filters, batch_norm, activation_fct, kernel_size):
    '''
        Convolution block cotaining two 2d convolutions each followed 
        by Batchnormalization and activation.
        
        Parameters:
        img:input image
        n_filters: number of filters for each convolution
        batch_norm: boolean, batch normalisation applied if True
        activation_fct: activation function name
        kernel_size: contains kernel size for the. convolutions
...
def build_unet(img, n_filters, dilate,dropout_down, dropout_up,
    ...
    '''
        Returns unet model
        parameters:
        img: input image, in our case of size 400x400x3
    '''

    # downwards path comprising of 4 convolutions blocks with increasing filters number
    ...
    # bottleneck path containing either a dilation block or a simple convolution block
    if (dilate):
        ...
        #adding all the dilated feature maps
    ...
    # upwards path contining 4 upsampling steps coupled with 4 concatenations and 4 convolution blocks
