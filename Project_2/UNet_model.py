...
def conv_batch(img, n_filters, batch_norm, activation_fct,kernel_size=(3, 3)):

    # Maybe try to add dropout between convolutions instead of outside

    # first convolution followed by batch normalization
    c1 = Conv2D(n_filters, kernel_size,kernel_initializer='he_normal', padding='same')(img)

    #if (batch_norm):
    #    c1 = BatchNormalization()(c1)
    #c1 = Activation(activation_fct)(c1)
    #activate_me=Activation(activation_fct)
    #c1=activate_me(c1)
...
def build_unet(img, n_filters, dropout_down=0.0, dropout_up=0.0, batch_norm=True, activation_fct='relu', final_activation='sigmoid', kernel_size=(3, 3)):
    ...
    up6 = Conv2DTranspose(n_filters*8, kernel_size, strides=(2, 2),padding='same', kernel_initializer='he_normal')(c5)
    ...
    up7 = Conv2DTranspose(n_filters*4, kernel_size, strides=(2, 2),padding='same', kernel_initializer='he_normal')(c6)
    up7 = concatenate([up7, c3])
    up7 = Dropout(dropout_up)(up7)
    c7 = conv_batch(up7, n_filters*4,batch_norm, activation_fct, kernel_size)

    up8 = Conv2DTranspose(n_filters*2, kernel_size, strides=(2, 2),padding='same', kernel_initializer='he_normal')(c7)
    up8 = concatenate([up8, c2])
    up8 = Dropout(dropout_up)(up8)
    c8 = conv_batch(up8, n_filters*2, batch_norm, activation_fct, kernel_size)

    up9 = Conv2DTranspose(n_filters, kernel_size, strides=(2, 2), padding='same', kernel_initializer='he_normal')(c8)
