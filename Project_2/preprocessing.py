...
def pad(img):
    ...
    return np.pad(img, pad_width = ((0, size_diff), (0, size_diff), (0,0)), mode='reflect')
...
def preprocess(root_dir='./training/',
    ...
    rdaug = RandAugment(5, 10, upscale_to_test_size)
    ...
    for (img_, gt_img_, file_, i) in zip(images, gt_images, files, range(len(images))):
        ...
        imgs_aug.append(pad(img_to_array(img_)))
        gt_imgs_aug.append(pad(img_to_array(gt_img_)))
        for j in range(2):
            ...
            if(upscale_to_test_size):
                size_diff = TEST_IMG_SIZE - img.shape[0]
                img = pad(img) #np.pad(img, pad_width = ((0, size_diff), (0, size_diff), (0,0)), mode='reflect')
                gt_img = pad (gt_img) #np.pad(gt_img, pad_width = ((0, size_diff), (0, size_diff), (0,0)), mode='reflect')
    ...
    if(divide_set == False):
        ...
        gt_imgs_aug = np.stack(gt_imgs_aug, axis = 0)
        ...
        X_test = np.stack(X_test, axis = 0)/ 255.0
